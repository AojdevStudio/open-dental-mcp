...
// --- Helper Functions --- (Keep your pollUntilCompleted if needed for older SDKs, but createAndPoll handles it for files)

// --- MCP Tools ---

// For type flexibility with OpenAI beta APIs
type OpenAIBetaAPI = typeof openai.beta & {
    vectorStores: {
        create: (params: { name: string }) => Promise<any>;
        list: () => Promise<{ data: any[] }>;
        files: {
            createAndPoll: (vectorStoreId: string, params: { file_id: string }) => Promise<any>;
            list: (vectorStoreId: string | string[]) => Promise<{ data: any[] }>;
        }
    }
};
const openAIBeta = openai.beta as OpenAIBetaAPI; // Cast for beta access

// 1. Create Vector Store
server.tool(
    "create_vector_store",
    { name: z.string().describe("A descriptive name for the vector store.") },
    async ({ name }) => {
        try {
...
server.tool(
    "upload_file",
    { filePath: z.string().describe("Path to the local file to upload (relative to project root).") },
    async ({ filePath }) => {
        try {
    ...
    async ({ query }) => {
        ...
        try {
            ...
            const messagesPage = await openai.beta.threads.messages.list(
                run.thread_id,
                { order: 'asc' } // Removed run_id filter here
            );
            ...
            if (!assistantMessages.length) {
             ...
             let responseText = '';
             const citations: { fileId: string, quote?: string, text: string, start_index: number, end_index: number }[] = []; // More detailed citation info
...
server.resource(
  "list_vector_stores",
  "vector_stores://", // URI format
  async (uri) => {
    try {
        const stores = await openAIBeta.vectorStores.list();
        const storeList = stores.data.map((s: any) => `- ${s.name} (ID: ${s.id}, Files: ${s.file_counts?.total || 0})`).join('\n'); // Added file counts
         return {
             contents: [{
                 uri: uri.href,
...
server.resource(
    ...
    new ResourceTemplate("vector_stores://{vectorStoreId}/files", { list: undefined }),
    async (uri, { vectorStoreId }) => {
        if (!vectorStoreId) {
        ...
        try {
            ...
            const fileList = files.data.map((f: any) => `- File ID: ${f.id}, Status: ${f.status}, Size: ${f.usage_bytes} bytes`).join('\n');
            return {
                contents: [{
                    uri: uri.href,
                     mimeType: "text/plain",
...
server.resource(
    "list_opendental_docs", // More specific name
